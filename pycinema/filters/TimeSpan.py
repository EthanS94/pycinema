...
class TimeSpan(Filter):
  ...
  def _update(self):
    ...
    year_to_year_pattern = re.compile(r'[0-9]+-[0-9]+\.nc$')
    just_year_pattern = re.compile(r'[0-9]+\.nc$')

    for g in nc_globs:
      for f in glob.glob(g):
        m = year_to_year_pattern.search(f)
        n = just_year_pattern.search(f)
        if m:
            start_s, end_s = m.group().split('.')[0].split('-')
            ...
            start_s = n.group().split('.')[0]
            s = datetime.strptime(str(start_s + '0101'), "%Y%m%d").date()
            e = datetime.strptime(str(start_s + '1231'), "%Y%m%d").date()
            start_date = s if start_date is None else min(start_date, s)
            end_date = e if end_date is None else max(end_date, e)
        else:
            continue
