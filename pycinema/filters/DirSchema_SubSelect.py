...
class DirSchema_SubSelect(Filter):
    def __init__(self):
        super().__init__(
            inputs={
                'directory': ''
...
def subdir_table(root):
    ...
    for p in root.glob("*"):
        ...
        if (p.is_dir()
            ...
            ds = xr.open_zarr(p, consolidated=None)
            ...
            if "obs_" in str(p):
                if "model" in ds.dims:
                    ...
                    model_names.append(re.split(r'_[0-9]', p.name)[0])
            ...
            # Historical Models
                ...
                if "model" in ds.dims:
                    ...
                    model_names.append(re.split(r'_historical', p.name)[0])
                hist_type = 'model'
                future_type = 'N/A'
